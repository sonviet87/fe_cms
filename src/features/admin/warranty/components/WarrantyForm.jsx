...
function WarrantyForm({ initialValue, onSubmit, itemValue, isEdit, fps }) {
  const validationRules = {
    ...
    start_day: yup.string().required('Xin chọn ngày bắt đầu'),
    end_day: yup.string().required('Xin chọn ngày kết thúc'),
    details: yup.lazy(() =>
      yup.array().of(
        yup.object({
  ...
  return (
    <Box component="form" noValidate autoComplete="off" onSubmit={handleSubmit(handleFormSubmit)}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12} md={6}>
          <BasicDatePicker name="start_day" lableText="Ngày bắt đầu" control={control} />
        </Grid>
        <Grid item xs={12} md={6}>
          <BasicDatePicker name="end_day" lableText="Ngày kết thúc" control={control} />
        </Grid>
                  ...
                  {fields.map((field, index) => (
                    <React.Fragment key={field.id}>
                      <TableRow>
                        ...
                        <TableCellStyled>
                          <TextFormik
                            name={`details[${index}].time_warranty`}

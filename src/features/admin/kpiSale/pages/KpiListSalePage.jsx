...
function KpiListSalePage() {
    ...
    const handleFilter = async (data) => {
        ...
        setLoading(true);
        try {
            const res = await kpiSaleAPI.getAll(data);
            if (res.status) {
    ...
    const handleKpiSetting = async (year) => {
        setLoading(true);
        const res = await kpiSetUpUserApi.getAll({'year':year});
        try {
    ...
    return (
        <WrapperPage>
            <TitleForm lable="KPI" />
            <KpiFilter loading={loading} filter={filter} onGetKpiSetting = {handleKpiSetting} onSubmit={handleFilter} memberGroup={memberGroup} setMemberGroup={setMemberGroup} selectedTypeKpi={selectedTypeKpi} setSelectedTypeKpi={setSelectedTypeKpi}   methods={methods}  />
...
export default KpiListSalePage;
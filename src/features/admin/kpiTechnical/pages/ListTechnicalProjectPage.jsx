...
import TechnicalProjectList from "../components/TechnicalProjectList";
...
function TechnicalProject() {
    ...
    const handleDelete = async (item) => {
        setLoading(true);
        const res = await technicalProjectAPI.delete([item.id]);
        if (res.status) {
            if (res.data.status) {
                setFilter({
                    per_page: 10,
    ...
    React.useEffect(() => {
        ...
        (async () => {
            setLoading(true);
            const res = await technicalProjectAPI.getAll(filter);
            try {
                if (res.status) {
    ...
    return (
        <WrapperPage>
            <TitleForm lable="Dự án" />
            <TechnicalProjectFilter loading={loading} filter={filter} onSubmit={handleFilter} users={users}   />
            {loading ? (
                <SkeletonList />
            ) : (<TechnicalProjectList list={list.list}

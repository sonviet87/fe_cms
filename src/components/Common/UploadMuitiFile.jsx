  ...
  return (
    <div>
      ...
      <label htmlFor={'btn-upload-muitl-' + index}>
        <input id={'btn-upload-muitl-' + index} style={{ display: 'none' }} type="file" multiple onChange={upload} />

        <TextFormik control={control} name={name} style={{ display: 'none' }} />
        <BasicButtonStyled className="btn-choose" variant="outlined" component="span" size="small">
      ...
      <div>
        {uploadedFiles.length !== 0 &&
          uploadedFiles.map((file, index) => {
            return (
              <WrapperBoxAlign isborder={false} key={index}>
                <Link href={process.env.REACT_APP_URL + file.url} target="_blank">
                  {file.name}
                </Link>
